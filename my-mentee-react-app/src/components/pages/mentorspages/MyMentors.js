...
export default function MyMentors() {
  ...
  // ── CHANGED: added error states for title and feedback name ──
  ...
  // ── CHANGED: title must contain at least some letters, not just numbers/symbols ──
  ...
  // ── CHANGED: feedback name only allows letters and spaces ──
  const handleFeedbackNameChange = (e) => {
    ...
    if (/^[a-zA-Z\s]*$/.test(value)) {
      ...
      setFeedbackNameError("Name must contain letters only.");
    }
  };
        ...
        <div className="bg-white p-8 rounded w-1/2 space-y-4">
          ...
          <div className="flex space-x-4">
            ...
            <div className="flex-1 space-y-2">
              ...
              {/* ── CHANGED: title uses handleTitleChange with letter validation ── */}
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
        <div className="bg-white p-8 rounded w-1/3 space-y-4">
          ...
          {/* ── CHANGED: feedback name now controlled and validates letters only ── */}
          <div className="space-y-1">
            <input
              value={feedbackName}
              onChange={handleFeedbackNameChange}
              placeholder="Full Name"
              className="border p-2 rounded w-full"
            />
            {feedbackNameError && (
              <p className="text-[11px] text-red-500">{feedbackNameError}</p>
            )}
          </div>
