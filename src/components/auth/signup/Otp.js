...
export default function OtpPage() {
  ...
  const [countdown, setCountdown]       = useState(5); // ← shows 5..4..3..2..1
  const [filling, setFilling]           = useState(true); // ← true while waiting
  const inputRefs = useRef([]);

  // ── generate code and wait 5s before auto-filling ──
  const generateAndFillOtp = () => {
    ...
    // fill boxes after 5 seconds
    setTimeout(() => {
      setOtp(randomCode.split(""));
      setFilling(false);
    }, 5000);
  };
      ...
      <div className="w-full lg:w-[58%] flex flex-col justify-between relative bg-white min-h-screen lg:min-h-0">
        ...
        <div className="max-w-xl w-full mx-auto px-6 sm:px-12 lg:px-16 py-12 my-auto space-y-6">
          ...
          <div className="space-y-2 text-center">
            ...
            {/* ── changes based on whether we're still waiting ── */}
            {filling ? (
              <p className="text-[11px] sm:text-xs text-gray-500 max-w-md mx-auto leading-relaxed">
                Your OTP is being sent to your email. It will appear automatically in{" "}
                <span className="font-bold text-[#C11224]">{countdown}s</span>...
              </p>
            ) : (
              <p className="text-[11px] sm:text-xs text-gray-500 max-w-md mx-auto leading-relaxed">
                Your OTP has been{" "}
                <span className="font-semibold text-gray-800">automatically filled</span>{" "}
                below. Simply click{" "}
                <span className="font-semibold text-gray-800">Proceed</span> to continue.
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              {filling && (
                <div className="flex items-center gap-1.5 pt-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#C11224] animate-bounce" style={{ animationDelay: "0ms" }} />
                  <span className="h-1.5 w-1.5 rounded-full bg-[#C11224] animate-bounce" style={{ animationDelay: "150ms" }} />
                  <span className="h-1.5 w-1.5 rounded-full bg-[#C11224] animate-bounce" style={{ animationDelay: "300ms" }} />
                </div>
              )}
            ...
            <div className="text-center">
              <span className="text-[11px] sm:text-xs text-gray-500 font-normal">
                ...
                <button
                  ...
                  disabled={filling}
                  className="text-[#C11224] font-bold hover:underline bg-transparent border-none p-0 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Click to Resend!
                </button>
              </span>
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={filling}
                className="w-full bg-[#C11224] hover:bg-[#A00F1E] disabled:opacity-50 disabled:cursor-not-allowed text-white py-3.5 px-4 rounded-xl font-bold text-xs sm:text-sm tracking-wide transition-all duration-200 active:scale-[0.99] cursor-pointer shadow-sm"
              >
                {filling ? `Sending OTP... (${countdown}s)` : "Proceed"}
              </button>
            </div>
          </form>
        </div>
