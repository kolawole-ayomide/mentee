...
export default function UploadPage() {
  ...
  const handleCheckboxChange = (id) => {
    setSelectedGrowthAreas((prev) => {
      // allow unchecking an already-selected box
      if (prev.includes(id)) {
        return prev.filter((item) => item !== id);
      }
      // block adding a 3rd selection
      if (prev.length >= 2) {
        return prev;
      }
      return [...prev, id];
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedGrowthAreas.length !== 2) {
      alert("Please select exactly 2 areas of growth.");
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2 text-left">
              ...
              <p className="text-[10px] text-gray-400 font-normal">
                Select exactly 2 areas ({selectedGrowthAreas.length}/2 selected)
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5 pt-1">
                {growthOptions.map((option) => {
                  const isChecked    = selectedGrowthAreas.includes(option.id);
                  const limitReached = selectedGrowthAreas.length >= 2 && !isChecked;
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        className="h-4 w-4 rounded border-gray-300 text-[#C11224] accent-[#C11224] disabled:cursor-not-allowed"
                      />
...
//     try {
//       // 1. Recover original account creation details saved in Step 1
//       const savedDataString = localStorage.getItem("vmpUser");
//       const existingUserData = savedDataString
//         ? JSON.parse(savedDataString)
//         : {};

//       // 2. CRITICAL FIX: Explicitly flat-merge all fields together so the email stays safe
//       const finalizedProfile = {
//         name: existingUserData.name || "",
//         email: existingUserData.email || "", // Carries the email value through safely
//         phone: existingUserData.phone || "",
//         staffId: existingUserData.staffId || "",
//         department: existingUserData.department || "",
//         designation: existingUserData.designation || "",
//         avatar: profileImage || null,
//         bio: bio.trim(),
//         password: password,
//         areasOfImprovement: selectedGrowthAreas.map((id) => {
...
//       // 3. Commit full combined object structure back to local database storage
//       localStorage.setItem("vmpUser", JSON.stringify(finalizedProfile));
//     } catch (error) {
...
//                 value={bio}
//                 onChange={(e) => setBio(e.target.value)}
//                 className="w-full text-xs p-3 text-gray-600 bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400 focus:ring-1 focus:ring-gray-400 resize-none font-normal"
//                 placeholder="Tell us about yourself..."
//                 required
//               />
//             </div>
