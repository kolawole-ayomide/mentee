// QR.js - Quarterly Review Form for Mentees
import React, { useState } from "react";
import { FiStar } from "react-icons/fi";
import { C } from "./data/profileData";

// ── Toggle this to switch between empty state and review form ──
// true  = shows "no mentor yet" empty state
// false = shows the actual quarterly review form
const SHOW_QR_EMPTY_STATE = false;
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button"
          ...
          <FiStar size={24} style={{
            ...
            stroke: star <= (hovered || value) ? "#FFCC28" : (error ? "#CF173C" : "#CBD5E1"),
...
function Question({ q, answers, setAnswers, error }) {
  return (
    <div className="space-y-2">
      ...
      {q.type === "rating" ? (
        ...
        <input
          ...
          className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#312F61]/20"
          style={{
            borderColor: error ? "#CF173C" : "#f1f5f9",
            background: error ? "#FFF5F5" : "#f8fafc",
          }}
        />
      )}
      {error && (
        <p className="text-[11px]" style={{ color: C.red }}>This question is required.</p>
      )}
    </div>
  );
}
...
export default function QR() {
  ...
  const [reviewSubmitted, setReviewSubmitted] = useState(() => {
    try {
      const raw = localStorage.getItem("vmpUser");
      const parsed = raw ? JSON.parse(raw) : null;
      return parsed?.reviewSubmitted === true;
    } catch {
      return false;
    }
  });

  const hasMentor = !SHOW_QR_EMPTY_STATE;

  const [page,    setPage]    = useState(0);
  const [answers, setAnswers] = useState({});
  const [errors,  setErrors]  = useState({}); // ── tracks which question ids are missing

  // ── No mentor assigned yet → show "no review" empty state ──
  if (!hasMentor) {
    return (
  ...
  // ── Review already submitted → show completion screen ──
  if (reviewSubmitted) {
    return (
  ...
  // ── gathers every question on the current page (both sections) ──
  const currentPageQuestions = [
    ...cur.questions,
    ...(cur.questions2 || []),
  ];

  // ── validates all questions on current page are answered ──
  ...
  const handleNext = () => {
    if (!validateCurrentPage()) return; // ── block navigation if invalid
    setErrors({});
    setPage((p) => p + 1);
  };
  ...
  const handleSubmit = () => {
    if (!validateCurrentPage()) return; // ── block submit if invalid

    // Save submission flag to localStorage
    try {
      const raw = localStorage.getItem("vmpUser");
      const parsed = raw ? JSON.parse(raw) : {};
      localStorage.setItem("vmpUser", JSON.stringify({ ...parsed, reviewSubmitted: true }));
    } catch {
      // fail silently
    }
    ...
    <div className="max-w-2xl space-y-5">
      ...
      {Object.keys(errors).length > 0 && (
        <div className="rounded-lg border px-3 py-2 text-xs"
          style={{ borderColor: C.red, background: "#FFF5F5", color: C.red }}>
      ...
      <div className="space-y-4">
        ...
        {cur.questions.map((q) => (
          <Question key={q.id} q={q} answers={answers} setAnswers={setAnswers} error={errors[q.id]} />
        ))}
      </div>
...
//     const raw = localStorage.getItem("vmpUser");
//     const parsed = raw ? JSON.parse(raw) : null;
//     return parsed?.mentor ? true : false;
//   } catch {
//     return false;
//   }
// })();
