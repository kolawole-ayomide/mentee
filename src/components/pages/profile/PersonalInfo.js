...
<div className="py-4 border-b border-[#DDDCF9] last:border-0">
    ...
    <div className="flex-1 min-w-0">
        ...
        <p className="text-sm" style={{ color: C.grey }}>{value}</p>
        )}
    </div>

    {/* Cancel / Edit — always on the right */}
    <div className="shrink-0 mt-1 min-w-[50px] text-right">
        {readOnly ? (
            // the disabled edit button
        <span className="text-xs font-semibold" style={{ color: "#CBD5E1" }}>Edit</span>
        ) : editing ? (
        <button onClick={handleCancel}
            className="text-xs font-semibold transition hover:opacity-70"
            style={{ color: C.red }}>Cancel</button>
        ) : (
        <button onClick={() => setEditing(true)}
            className="text-xs font-semibold transition hover:opacity-70"
            style={{ color: C.red }}>Edit</button>
        )}
    </div>
    </div>
</div>
...
const update = (field) => (val) => setUser((p) => ({ ...p, [field]: val }));
...
<>
    ...
    <div className="flex flex-col items-center lg:items-start gap-4 w-full lg:w-[40%] shrink-0 pr-0 lg:pr-10 pb-8 lg:pb-0">
        ...
        {!avatarErr && user.avatar ? (
            ...
            <div className="h-28 w-28 rounded-full flex items-center justify-center text-white text-3xl font-bold shadow"
            style={{ background: C.navy }}>
        ...
        <p className="text-xs mt-0.5" style={{ color: C.grey }}>{user.workEmail}</p>
        <p className="text-xs" style={{ color: C.grey }}>{user.staffId}</p>
        </div>

        {/* bio */}
        <div className="w-full">
        <p className="text-xs font-bold mb-1" style={{ color: C.dark }}>Bio/Summary</p>
        <p className="text-xs leading-relaxed" style={{ color: C.grey }}>{user.bio}</p>
        </div>
    </div>
    ...
    <div className="w-full lg:w-[60%] lg:pr-12 lg:pl-7 divide-y divide-[#DDDCF9]">
        ...
        <div className="flex items-start justify-between gap-4">
            ...
            style={{ color: C.red }}>Update</button>
        </div>
        </div>
    </div>
    </div>
    ...
    <AreaOfImprovement
        ...
        onSave={(areas) => { update("areasOfImprovement")(areas); setShowImprovement(false); }}
    />
    )}
</>
);
